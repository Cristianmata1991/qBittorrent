...
void TrackerListWidget::loadTrackers()
{
    ...
    const auto updateMinMax = [](std::pair<int, int> &minMax, int val)
    {
        if (val == -1)
            return;
        minMax.first = (minMax.first == -1) ? val
                                            : std::min(minMax.first, val);
        minMax.second = (minMax.second == -1) ? val
                                              : std::max(minMax.second, val);
    };
    const auto printMinMax = [numberIf](const std::pair<int, int> &minMax)
    {
        if (minMax.first == minMax.second)
            return numberIf(minMax.first);
        else
    ...
    for (const BitTorrent::TrackerEntry &entry : asConst(torrent->trackers()))
    {
        ...
        {
            updateMinMax(peersMinMax, endpoint.numPeers);
        ...
        item->setText(COL_DOWNLOADED, printMinMax(downloadedMinMax));
        setAlignment(item);
    }
